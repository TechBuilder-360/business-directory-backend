package repository

import (
	"context"
	"github.com/TechBuilder-360/business-directory-backend/models"
	"gorm.io/gorm"
)

//go:generate mockgen -destination=../mocks/repository/user.go -package=repository github.com/TechBuilder-360/business-directory-backend/repository UserRepository
type UserRepository interface {
	DoesUserEmailExist(string) (bool, error)
	Create(user *models.UserProfile) error
	Get(user *models.UserProfile) error
	GetByEmail(user *models.UserProfile) error
	Update() error
	Deactivate() error
	Activate() error
	WithTx(tx *gorm.DB) UserRepository
}

type DefaultUserRepo struct {
	db *gorm.DB
}



func (r *DefaultUserRepo) GetByEmail(user *models.UserProfile) error {
	panic("implement me")
}

func NewUserRepository(db *gorm.DB) UserRepository {
	return &DefaultUserRepo{
		db: db,
	}
}

func (r *DefaultUserRepo) WithTx(tx *gorm.DB) UserRepository {
	return NewUserRepository(tx)
}

func (r *DefaultUserRepo) Deactivate() error {
	panic("implement me")
}

func (r *DefaultUserRepo) Activate() error {
	panic("implement me")
}

func (r *DefaultUserRepo) Update() error {
	panic("implement me")
}

// DoesUserEmailExist ...
func (r *DefaultUserRepo) DoesUserEmailExist(email string) (bool, error) {
	panic("not implemented")
	return true, nil
}

// Create ...
func (r *DefaultUserRepo) Create(user *models.UserProfile) error {
	return r.db.WithContext(context.Background()).Create(user).Error
}

// Get returns user profile
func (r *DefaultUserRepo) Get(user *models.UserProfile) error {
	panic("not implemented")
	return nil
}
