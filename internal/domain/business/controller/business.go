...
func (c *BusinessController) RegisterRoutes(router *fiber.App) {
	...
	//apis.HandleFunc("", middlewares.CacheClient.Middleware(middlewares.Adapt(http.HandlerFunc(c.GetAllBusiness), middlewares.AuthorizeUserJWT())).ServeHTTP).Methods(http.MethodGet)
	//apis.HandleFunc("", middlewares.Adapt(http.HandlerFunc(c.CreateBusiness), middlewares.AuthorizeUserJWT()).ServeHTTP).Methods(http.MethodPost)
	//apis.HandleFunc("/{id}", middlewares.CacheClient.Middleware(middlewares.Adapt(http.HandlerFunc(c.GetSingleBusiness), middlewares.AuthorizeUserJWT())).ServeHTTP).Methods(http.MethodGet)
	//apis.HandleFunc("/activate", middlewares.Adapt(http.HandlerFunc(c.ChangeActiveStatus), middlewares.AuthorizeUserJWT(), middlewares.AuthorizeBusinessJWT).ServeHTTP).Methods(http.MethodPatch)
}
...
//	Business, err := middlewares.BusinessFromContext(r)
//	if err != nil {
//		logger.Error(err.Error())
//		w.WriteHeader(http.StatusBadRequest)
//		json.NewEncoder(w).Encode(utils.ErrorResponse{
